...
function routes(app, onlineUsers) {
    ...
    app.get('/api/messages/:roomId/:userId', async (req, res) => {
        console.log(`GET REQUEST: fetching previous messages for room ${req.params.roomId} userId = ${req.params.userId}`);
        const data = await messages.getRoomMsgs(req.params.roomId,req.params.userId);

        console.table(data);
        res.send(data);
    })

    // request previous my message
    app.get('/api/my_messages/:roomId/:userId', async (req, res) => {
        console.log(`GET REQUEST: fetching previous messages for room ${req.params.roomId} userId = ${req.params.userId}`);
        const myData = await messages.getMyRoomMsgs(req.params.roomId,req.params.userId);
